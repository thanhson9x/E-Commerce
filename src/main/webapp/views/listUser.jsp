				...
				<div class="row">
					...
					<div class="col-lg-8 col-md-7 col-12">
						<div class="search-bar-top">
							<form
								action="${pageContext.request.contextPath}/admin/allUsers/search"
								method="get">
								<div class="search-bar">
									<select name="role">
										<option value="0">All Role</option>
										<c:if test="${listRole ne null && listRole.size() gt 0}">
											<c:forEach items="${listRole }" var="item">
												<option value="${item.id }">${item.name }</option>
											</c:forEach>
										</c:if>
									</select> <input name="keyword" value=""
										placeholder="Search by user name or email..." type="search">
				...
				<div class="col-12">
					<div class="bread-inner">
						<ul class="bread-list">
							...
							<li><a href="${pageContext.request.contextPath}/admin/info">Admin</a></li>
						</ul>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-12">
					...
					<table class="table shopping-summery">
						...
						<tbody>
							<c:choose>
								<c:when test="${listUser ne null && listUser.size() gt 0}">
									<c:forEach items="${listUser }" var="item">
										<tr>
											<td class="image" data-title="No"><img
												src="https://via.placeholder.com/100x100" alt="#"></td>
											<td><p class="text-center">
													<c:forEach items="${item.roles }" var="i">
													${i.name }
													<br>
													</c:forEach>
													<c:if test="${item.id gt '1'}">
														<form
															action="${pageContext.request.contextPath}/admin/allUsers/submitRole"
															method="post">
															<input type="hidden" name="id" value="${item.id }" /> <select
																name="role">
																<c:forEach items="${listRole }" var="j">
																	<option value="${j.id }">${j.name }</option>
																</c:forEach>
															</select> <input class="btn animate" type="submit" value="Change" />
														</form>
													</c:if>
												</p></td>
											<td class="product-des" data-title="Description">
												...
												<p class="product-des">${item.lastName }</p>
											</td>
											<td class="price" data-title="Price"><span>${item.dob}</span></td>
											<td class="price" data-title="Price"><span>${item.gender.name}
											</span></td>
											<td class="total-amount" data-title="Total"><a
												href="${pageContext.request.contextPath}/admin/updateUser?id=${item.id }">Update</a></td>
											<c:if test="${item.id gt '1'}">
												<td class="action" data-title="Remove"><a
													href="${pageContext.request.contextPath}/admin/deleteUser?id=${item.id }"><i
														class="ti-trash remove-icon"></i></a></td>
											</c:if>
										</tr>
									</c:forEach>
									<tr>
										<td colspan="7"><c:import url="common/paging.jsp" /></td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr>
