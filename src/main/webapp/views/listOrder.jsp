				...
				<div class="row">
					...
					<div class="col-lg-8 col-md-7 col-12">
						<div class="search-bar-top">
							...
							<sec:authorize access="hasRole('ADMIN')">
								<form
									action="${pageContext.request.contextPath}/admin/allOrders/search"
									method="get">
									<div class="search-bar">
										<select name="category">
											<option value="0">All Category</option>
											...
											placeholder="Search order by customer name or status..."
											type="search">
										<button class="btnn" value="search" type="submit">
											<i class="ti-search"></i>
										</button>
							...
							<sec:authorize access="hasRole('CUSTOMER')">
								<form
									action="${pageContext.request.contextPath}/customer/allOrders/search"
									method="get">
									<div class="search-bar">
										<select name="category">
											<option value="0">All Category</option>
										...
										</select> <input name="keyword" placeholder="Search order by status..."
											type="search">
										<button class="btnn" value="search" type="submit">
											<i class="ti-search"></i>
										</button>
						...
						<ul class="bread-list">
							...
							<sec:authorize access="hasRole('ADMIN')">
								...
								<li><a href="${pageContext.request.contextPath}/admin/info">Admin<i
										class="ti-arrow-right"></i></a></li>
								<li class="active"><a
									href="${pageContext.request.contextPath}/admin/allOrders">All
										Orders</a></li>
							</sec:authorize>
							...
							<sec:authorize access="hasRole('CUSTOMER')">
								...
								<li><a
									href="${pageContext.request.contextPath}/customer/info">${sessionScope.user.firstName}<i
										class="ti-arrow-right"></i></a></li>
								<li class="active"><a
									href="${pageContext.request.contextPath}/customer/allOrders">All
										Orders</a></li>
							</sec:authorize>
			...
			<div class="row">
				<div class="col-12">
					...
					<table class="table shopping-summery">
						...
						<tbody>
							<c:choose>
								<c:when test="${listOrder ne null && listOrder.size() gt 0}">
									<c:forEach items="${listOrder }" var="item">
										...
										<sec:authorize access="hasRole('ADMIN')">
											<tr>
												...
												<td class="product-des" data-title="Description">
													<p class="product-name">${item.user.firstName }</p>
													<p class="product-des">${item.user.lastName }</p>
												</td>

												<td class="product-des" data-title="Description"><form
														action="${pageContext.request.contextPath}/admin/allOrders/submitStatus"
														method="post">
														<input type="hidden" name="id" value="${item.id }" /> <input
															type="text" name="status" value="${item.status }" /> <input
															class="btn animate" type="submit" value="Change" />
													</form></td>

												<td class="price" data-title="Price"><span>${item.dateCreated}</span></td>

												<td class="product-des" data-title="Description"><c:choose>
														<c:when
															test="${item.orderDetails ne null && item.orderDetails.size() gt 0}">
															<c:forEach items="${item.orderDetails }" var="i">
																<p>
																	<a
																		href="${pageContext.request.contextPath}/admin/product/${i.product.id}">${i.product.name }</a>
																</p>
																<p>$ ${i.unitPrice } x ${i.quantity }</p>
															</c:forEach>
														</c:when>
														<c:otherwise>
															<td class="text-center">Data not found!</td>
												...
												<td class="price" data-title="Price"><span>${item.orderPrice}
												</span></td>

												<td class="action" data-title="Remove"><a
													href="${pageContext.request.contextPath}/admin/deleteOrder?id=${item.id }"><i
										...
										<sec:authorize access="hasRole('CUSTOMER')">
											<tr>
												...
												<td class="product-des" data-title="Description"><c:choose>
														<c:when test="${item.status eq 'unpaid'}">
															<p>${item.status }</p>
															<a
																href="${pageContext.request.contextPath}/customer/checkout/${item.id }"
																class="btn">PAY</a>
														</c:when>
														<c:otherwise>
															<p>${item.status }</p>
												...
												<td class="product-des" data-title="Description"><c:choose>
														<c:when
															...
															<c:forEach items="${item.orderDetails }" var="i">
																<p>
																	<a
																		href="${pageContext.request.contextPath}/customer/product/${i.product.id}">${i.product.name }</a>
																</p>
																<p>$ ${i.unitPrice } x ${i.quantity }</p>
															</c:forEach>
														</c:when>
												...
												<c:if test="${item.status eq 'unpaid'}">
													<td class="action" data-title="Remove"><a
														href="${pageContext.request.contextPath}/customer/deleteOrder?id=${item.id }"><i
															class="ti-trash remove-icon"></i></a></td>
												</c:if>
